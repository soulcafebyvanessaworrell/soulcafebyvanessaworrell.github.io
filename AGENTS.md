...
A translation replaces exactly one file, `src/i18n/<code>.ts`, and adds `src/content/blog/welcome-<code>.md`, which is mandatory because the blog index has no empty state. It touches nothing else.
...
- After changing any dictionary or post of a nastaliq, naskh, or devanagari locale, run `bun scripts/subset-fonts.mts` (needs python3 fontTools and brotli) and commit `src/assets/fonts`.
