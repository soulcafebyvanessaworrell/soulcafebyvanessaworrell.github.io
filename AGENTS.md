...
| `LanguagePicker.astro` | `locale`, `available?`, `fallbackPath?`, `class?` | A no-JS `<details>` disclosure. The summary shows a globe and the current endonym from `sm` up, and only the uppercase code below `sm`; the panel lists every locale by region, each in its own script, no flags. Locales not in `available` link to `fallbackPath`. |
